...
1. **Everything ships in the workspace, reproducible from a clean environment.** Do not satisfy a request for data, pages, fragments, or styling by mutating only the running instance — write it into the tree first, then apply it. A site that cannot be rebuilt from what is checked into the workspace is not finished, however correct it looks in the browser.
	...
	So the reproducible unit is the workspace, not a single directory. Express what the tree can carry unaided — style book, master page, fragment CSS, `layout-set` settings — through the tree, since none of it needs reattaching. A rebrand also **ships a sibling `frontend` project** carrying `themeCSS` for the Clay level overrides that have no style book token, and `themeFavicon` for the browser tab icon — a favicon has no style book equivalent, so there is no other way to deliver one. Build and deploy it as part of the work; the only caveat is that selecting it is a manual step in Site Administration → Design → Theme on this build, which is lost on reprovision (`skills/theme-and-design/SKILL.md` → "Apply to Site"). Say that plainly in the handover rather than skipping the project — an undelivered theme extension is a gap in the work, not a simplification.
